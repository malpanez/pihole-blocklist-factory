...
def test_compute_recommendations_success(tmp_path: Path) -> None:
    ...
    aggregates = {
        ...
        "overlap_3_plus": 0,
        "per_source": {
            "s1": {"total_contributions": 2, "unique_domains": 1},
            "s2": {"total_contributions": 2, "unique_domains": 1},
        },
    }
    marginal = {"s1": 1, "s2": 1}
...
def test_compute_recommendations_invalid_aggregates(tmp_path: Path) -> None:
    dist_dir = tmp_path / "dist"
...
def test_compute_recommendations_no_marginal(tmp_path: Path) -> None:
    ...
    aggregates = {
        ...
        "overlap_2": 0,
        "overlap_3_plus": 0,
        "per_source": {"s1": {"total_contributions": 1, "unique_domains": 1}},
    }
    _write(dist_dir / "reports" / "provenance_aggregates.json", json.dumps(aggregates))
    result = compute_recommendations(dist_dir, _settings())
...
def test_compute_recommendations_moderate_and_low(tmp_path: Path) -> None:
    dist_dir = tmp_path / "dist"
    # Total domains = 1000: 997 shared between s1/s2, 2 unique to s1, 1 unique to s2
    aggregates = {
        "total_unique": 1000,
        "overlap_2": 997,
        "overlap_3_plus": 0,
        "per_source": {
            "s1": {"total_contributions": 999, "unique_domains": 2},
            "s2": {"total_contributions": 998, "unique_domains": 1},
        },
    }
